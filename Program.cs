...
class Program
{
    ...
    static void SQLWriteForTCP(uint[] DataArray)
    {
        ...
        try
        {
            ...
            uint[] oldDataArray = new uint[150];
            int i = 0;
            while (sqlDataReader.Read())
            {
                //sqlDataReader.NextResult();
                uint tempSql = (uint)sqlDataReader.GetInt32(0);
                // Console.WriteLine(tempSql);
                if (tempState == true)
                {
                    oldDataArray[i] = tempSql;
                    i++;
                }
                tempState = true;
            }
            if (tempState == false)
            {
                Array.Clear(oldDataArray, 0, oldDataArray.Length);
            }
            DynamicUpdate(oldDataArray, DataArray);
            sqlDataReader.Close();
            if (tempState)
            {
    ...
    static async Task ConfigDataBasue_Async()
    {
    ...
    static void DynamicUpdate(uint[] oldArray, uint[] newArray)
    {
        int i = 0;
        bool endUpdateData = false;
